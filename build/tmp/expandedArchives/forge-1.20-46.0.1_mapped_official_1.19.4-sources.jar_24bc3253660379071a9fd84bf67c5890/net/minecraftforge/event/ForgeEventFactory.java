...
public class ForgeEventFactory
{
    ...
    public static int onItemExpire(ItemEntity entity, @NotNull ItemStack item)
    {
        ...
        ItemExpireEvent event = new ItemExpireEvent(entity, (item.isEmpty() ? 6000 : item.getItem().getEntityLifespan(item, entity.getLevel())));
        if (!MinecraftForge.EVENT_BUS.post(event)) return -1;
    ...
    {
        boolean isCanceled = MinecraftForge.EVENT_BUS.post(new EntityMountEvent(entityMounting, entityBeingMounted, entityMounting.getLevel(), isMounting));
    ...
    public static boolean fireSleepingLocationCheck(LivingEntity player, BlockPos sleepingLocation)
    {
        ...
        if (canContinueSleep == Result.DEFAULT)
        {
            return player.getSleepingPos().map(pos-> {
                ...
                return state.getBlock().isBed(state, player.getLevel(), pos, player);
            }).orElse(false);
        }
        else
