...
class DimensionsCommand
{
    static ArgumentBuilder<CommandSourceStack, ?> register()
    {
        return Commands.literal("dimensions")
            ...
            .executes(ctx -> {
                ctx.getSource().m_288197_(() -> Component.translatable("commands.forge.dimensions.list"), true);
                ...
                types.keySet().stream().sorted().forEach(key -> {
                    ctx.getSource().m_288197_(() -> Component.literal(key + ": " + types.get(key).stream().map(ResourceLocation::toString).sorted().collect(Collectors.joining(", "))), false);
                });
                return 0;
            });
    }
