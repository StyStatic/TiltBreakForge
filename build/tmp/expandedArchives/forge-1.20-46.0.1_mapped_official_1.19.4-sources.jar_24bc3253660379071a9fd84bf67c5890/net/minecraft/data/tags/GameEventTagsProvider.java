...
public class GameEventTagsProvider extends IntrinsicHolderTagsProvider<GameEvent> {
   ...
   protected void addTags(HolderLookup.Provider p_255981_) {
      this.tag(GameEventTags.VIBRATIONS).add(VIBRATIONS_EXCEPT_FLAP).add(VibrationSystem.f_279664_).add(GameEvent.FLAP);
      this.tag(GameEventTags.SHRIEKER_CAN_LISTEN).add(GameEvent.SCULK_SENSOR_TENDRILS_CLICKING);
      this.tag(GameEventTags.WARDEN_CAN_LISTEN).add(VIBRATIONS_EXCEPT_FLAP).add(VibrationSystem.f_279664_).add(GameEvent.SHRIEK).addTag(GameEventTags.SHRIEKER_CAN_LISTEN);
