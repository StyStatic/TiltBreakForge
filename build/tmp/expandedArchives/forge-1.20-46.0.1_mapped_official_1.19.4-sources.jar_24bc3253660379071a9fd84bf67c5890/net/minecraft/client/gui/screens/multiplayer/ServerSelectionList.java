...
public class ServerSelectionList extends ObjectSelectionList<ServerSelectionList.Entry> {
   ...
   public static class LANHeader extends ServerSelectionList.Entry {
      ...
      public void render(GuiGraphics p_281475_, int p_282477_, int p_282819_, int p_282001_, int p_281911_, int p_283126_, int p_282303_, int p_281998_, boolean p_282625_, float p_281811_) {
         ...
         p_281475_.m_280614_(this.minecraft.font, ServerSelectionList.SCANNING_LABEL, this.minecraft.screen.width / 2 - this.minecraft.font.width(ServerSelectionList.SCANNING_LABEL) / 2, i, 16777215, false);
         String s = LoadingDotsText.get(Util.getMillis());
         p_281475_.m_280056_(this.minecraft.font, s, this.minecraft.screen.width / 2 - this.minecraft.font.width(s) / 2, i + 9, 8421504, false);
      }
   ...
   public static class NetworkServerEntry extends ServerSelectionList.Entry {
      ...
      public void render(GuiGraphics p_282600_, int p_282649_, int p_283641_, int p_282277_, int p_283034_, int p_281533_, int p_282746_, int p_281454_, boolean p_283673_, float p_282694_) {
         p_282600_.m_280614_(this.minecraft.font, LAN_SERVER_HEADER, p_282277_ + 32 + 3, p_283641_ + 1, 16777215, false);
         p_282600_.m_280056_(this.minecraft.font, this.serverData.getMotd(), p_282277_ + 32 + 3, p_283641_ + 12, 8421504, false);
         if (this.minecraft.options.hideServerAddress) {
            p_282600_.m_280614_(this.minecraft.font, HIDDEN_ADDRESS_TEXT, p_282277_ + 32 + 3, p_283641_ + 12 + 11, 3158064, false);
         } else {
            p_282600_.m_280056_(this.minecraft.font, this.serverData.getAddress(), p_282277_ + 32 + 3, p_283641_ + 12 + 11, 3158064, false);
         }
   ...
   public class OnlineServerEntry extends ServerSelectionList.Entry {
      ...
      protected OnlineServerEntry(JoinMultiplayerScreen p_99864_, ServerData p_99865_) {
         ...
         this.minecraft = Minecraft.getInstance();
         this.icon = FaviconTexture.m_289187_(this.minecraft.getTextureManager(), p_99865_.ip);
      }
      ...
      public void render(GuiGraphics p_281406_, int p_281506_, int p_282921_, int p_281363_, int p_283596_, int p_281630_, int p_283567_, int p_282864_, boolean p_282999_, float p_281423_) {
         ...
         p_281406_.m_280056_(this.minecraft.font, this.serverData.name, p_281363_ + 32 + 3, p_282921_ + 1, 16777215, false);
         List<FormattedCharSequence> list = this.minecraft.font.split(this.serverData.motd, p_283596_ - 32 - 2);

         for(int i = 0; i < Math.min(list.size(), 2); ++i) {
            p_281406_.m_280649_(this.minecraft.font, list.get(i), p_281363_ + 32 + 3, p_282921_ + 12 + 9 * i, 8421504, false);
         }
         ...
         p_281406_.m_280614_(this.minecraft.font, component1, p_281363_ + p_283596_ - j - 15 - 2, p_282921_ + 1, 8421504, false);
         int k = 0;
         int l;
         ...
         p_281406_.m_280163_(ServerSelectionList.f_279631_, p_281363_ + p_283596_ - 15, p_282921_, (float)(k * 10), (float)(176 + l * 8), 10, 8, 256, 256);
         ...
         this.m_280396_(p_281406_, p_281363_, p_282921_, this.icon.m_289196_());
         ...
         if (this.minecraft.options.touchscreen().get() || p_282999_) {
            ...
            if (this.canJoin()) {
               if (k1 < 32 && k1 > 16) {
                  p_281406_.m_280163_(ServerSelectionList.ICON_OVERLAY_LOCATION, p_281363_, p_282921_, 0.0F, 32.0F, 32, 32, 256, 256);
               } else {
                  p_281406_.m_280163_(ServerSelectionList.ICON_OVERLAY_LOCATION, p_281363_, p_282921_, 0.0F, 0.0F, 32, 32, 256, 256);
               }
            }

            if (p_281506_ > 0) {
               if (k1 < 16 && l1 < 16) {
                  p_281406_.m_280163_(ServerSelectionList.ICON_OVERLAY_LOCATION, p_281363_, p_282921_, 96.0F, 32.0F, 32, 32, 256, 256);
               } else {
                  p_281406_.m_280163_(ServerSelectionList.ICON_OVERLAY_LOCATION, p_281363_, p_282921_, 96.0F, 0.0F, 32, 32, 256, 256);
               }
            }

            if (p_281506_ < this.screen.getServers().size() - 1) {
               if (k1 < 16 && l1 > 16) {
                  p_281406_.m_280163_(ServerSelectionList.ICON_OVERLAY_LOCATION, p_281363_, p_282921_, 64.0F, 32.0F, 32, 32, 256, 256);
               } else {
                  p_281406_.m_280163_(ServerSelectionList.ICON_OVERLAY_LOCATION, p_281363_, p_282921_, 64.0F, 0.0F, 32, 32, 256, 256);
               }
            }
         }
      ...
      private boolean uploadServerIcon(@Nullable byte[] p_273176_) {
         if (p_273176_ == null) {
            ...
            try {
               this.icon.m_289201_(NativeImage.read(p_273176_));
