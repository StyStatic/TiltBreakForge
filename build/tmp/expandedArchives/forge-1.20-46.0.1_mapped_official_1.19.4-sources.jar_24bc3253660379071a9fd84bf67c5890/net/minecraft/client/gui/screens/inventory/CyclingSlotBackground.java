   ...
   public void render(AbstractContainerMenu p_267293_, GuiGraphics p_282894_, float p_266785_, int p_266711_, int p_266841_) {
      ...
      if (!this.icons.isEmpty() && !slot.hasItem()) {
         ...
         if (f < 1.0F) {
            ...
            this.m_280347_(slot, this.icons.get(i), 1.0F - f, p_282894_, p_266711_, p_266841_);
         }

         this.m_280347_(slot, this.icons.get(this.iconIndex), f, p_282894_, p_266711_, p_266841_);
      }
   }

   private void m_280347_(Slot p_283532_, ResourceLocation p_283004_, float p_282627_, GuiGraphics p_282825_, int p_281375_, int p_283041_) {
      TextureAtlasSprite textureatlassprite = Minecraft.getInstance().getTextureAtlas(TextureAtlas.LOCATION_BLOCKS).apply(p_283004_);
      p_282825_.m_280565_(p_281375_ + p_283532_.x, p_283041_ + p_283532_.y, 0, 16, 16, textureatlassprite, 1.0F, 1.0F, 1.0F, p_282627_);
   }
