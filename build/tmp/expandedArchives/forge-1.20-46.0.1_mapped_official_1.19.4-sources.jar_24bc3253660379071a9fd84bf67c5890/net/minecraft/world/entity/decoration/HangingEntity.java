...
public abstract class HangingEntity extends Entity {
      ...
      if (!this.getLevel().noCollision(this)) {
         ...
         for(int k = 0; k < i; ++k) {
            for(int l = 0; l < j; ++l) {
               ...
               BlockState blockstate = this.getLevel().getBlockState(blockpos$mutableblockpos);
               if (net.minecraft.world.level.block.Block.canSupportCenter(this.getLevel(), blockpos$mutableblockpos, this.direction))
                  continue;
               if (!blockstate.m_280296_() && !DiodeBlock.isDiode(blockstate)) {
                  return false;
               }
            }
         }

         return this.getLevel().getEntities(this, this.getBoundingBox(), HANGING_ENTITY).isEmpty();
      }
   }
   ...
   public boolean skipAttackInteraction(Entity p_31750_) {
      if (p_31750_ instanceof Player player) {
         return !this.getLevel().mayInteract(player, this.pos) ? true : this.hurt(this.damageSources().playerAttack(player), 0.0F);
      } else {
         return false;
      }
   }
   ...
   public void move(MoverType p_31719_, Vec3 p_31720_) {
      if (!this.getLevel().isClientSide && !this.isRemoved() && p_31720_.lengthSqr() > 0.0D) {
         this.kill();
         this.dropItem((Entity)null);
      }
   ...
   public void push(double p_31744_, double p_31745_, double p_31746_) {
      if (!this.getLevel().isClientSide && !this.isRemoved() && p_31744_ * p_31744_ + p_31745_ * p_31745_ + p_31746_ * p_31746_ > 0.0D) {
         this.kill();
         this.dropItem((Entity)null);
      }
   ...
   public ItemEntity spawnAtLocation(ItemStack p_31722_, float p_31723_) {
      ItemEntity itementity = new ItemEntity(this.getLevel(), this.getX() + (double)((float)this.direction.getStepX() * 0.15F), this.getY() + (double)p_31723_, this.getZ() + (double)((float)this.direction.getStepZ() * 0.15F), p_31722_);
