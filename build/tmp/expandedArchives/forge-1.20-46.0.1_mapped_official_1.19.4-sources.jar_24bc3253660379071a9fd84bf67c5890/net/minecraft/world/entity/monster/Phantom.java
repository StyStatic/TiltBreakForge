...
public class Phantom extends FlyingMob implements Enemy {
   ...
   public void tick() {
      ...
      if (this.getLevel().isClientSide) {
         ...
         if (f > 0.0F && f1 <= 0.0F) {
            this.getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), SoundEvents.PHANTOM_FLAP, this.getSoundSource(), 0.95F + this.random.nextFloat() * 0.05F, 0.95F + this.random.nextFloat() * 0.05F, false);
         }
         ...
         this.getLevel().addParticle(ParticleTypes.MYCELIUM, this.getX() + (double)f2, this.getY() + (double)f4, this.getZ() + (double)f3, 0.0D, 0.0D, 0.0D);
         this.getLevel().addParticle(ParticleTypes.MYCELIUM, this.getX() - (double)f2, this.getY() + (double)f4, this.getZ() - (double)f3, 0.0D, 0.0D, 0.0D);
      }
   ...
   class PhantomAttackPlayerTargetGoal extends Goal {
      ...
      public boolean canUse() {
         if (this.nextScanTick > 0) {
            ...
            List<Player> list = Phantom.this.getLevel().getNearbyPlayers(this.attackTargeting, Phantom.this, Phantom.this.getBoundingBox().inflate(16.0D, 64.0D, 16.0D));
            if (!list.isEmpty()) {
   ...
   class PhantomAttackStrategyGoal extends Goal {
      ...
      public void stop() {
         Phantom.this.anchorPoint = Phantom.this.getLevel().getHeightmapPos(Heightmap.Types.MOTION_BLOCKING, Phantom.this.anchorPoint).above(10 + Phantom.this.random.nextInt(20));
      }
      ...
      private void setAnchorAboveTarget() {
         ...
         if (Phantom.this.anchorPoint.getY() < Phantom.this.getLevel().getSeaLevel()) {
            Phantom.this.anchorPoint = new BlockPos(Phantom.this.anchorPoint.getX(), Phantom.this.getLevel().getSeaLevel() + 1, Phantom.this.anchorPoint.getZ());
         }
   ...
   class PhantomCircleAroundAnchorGoal extends Phantom.PhantomMoveTargetGoal {
      ...
      public void tick() {
         ...
         if (Phantom.this.moveTargetPoint.y < Phantom.this.getY() && !Phantom.this.getLevel().isEmptyBlock(Phantom.this.blockPosition().below(1))) {
            this.height = Math.max(1.0F, this.height);
            this.selectNext();
         }

         if (Phantom.this.moveTargetPoint.y > Phantom.this.getY() && !Phantom.this.getLevel().isEmptyBlock(Phantom.this.blockPosition().above(1))) {
         ...
         if (livingentity == null) {
            ...
            if (!this.canUse()) {
               ...
               if (Phantom.this.tickCount > this.catSearchTick) {
                  ...
                  List<Cat> list = Phantom.this.getLevel().getEntitiesOfClass(Cat.class, Phantom.this.getBoundingBox().inflate(16.0D), EntitySelector.ENTITY_STILL_ALIVE);
         ...
         if (livingentity != null) {
            ...
            if (Phantom.this.getBoundingBox().inflate((double)0.2F).intersects(livingentity.getBoundingBox())) {
               ...
               if (!Phantom.this.isSilent()) {
                  Phantom.this.getLevel().levelEvent(1039, Phantom.this.blockPosition(), 0);
               }
