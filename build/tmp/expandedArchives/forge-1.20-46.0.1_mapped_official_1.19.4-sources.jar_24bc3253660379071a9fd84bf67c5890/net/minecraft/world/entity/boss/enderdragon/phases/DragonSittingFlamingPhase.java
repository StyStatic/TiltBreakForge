...
public class DragonSittingFlamingPhase extends AbstractDragonSittingPhase {
   ...
   public void doClientTick() {
      ...
      if (this.flameTicks % 2 == 0 && this.flameTicks < 10) {
         ...
         for(int i = 0; i < 8; ++i) {
            ...
            for(int j = 0; j < 6; ++j) {
               this.dragon.getLevel().addParticle(ParticleTypes.DRAGON_BREATH, d3, d4, d5, -vec3.x * (double)0.08F * (double)j, -vec3.y * (double)0.6F, -vec3.z * (double)0.08F * (double)j);
            }
   ...
   public void doServerTick() {
      ...
      if (this.flameTicks >= 200) {
         ...
         while(this.dragon.getLevel().isEmptyBlock(blockpos$mutableblockpos)) {
            --d3;
            if (d3 < 0.0D) {
               d3 = d2;
               break;
         ...
         this.flame = new AreaEffectCloud(this.dragon.getLevel(), d0, d3, d1);
         this.flame.setOwner(this.dragon);
         this.flame.setRadius(5.0F);
         this.flame.setDuration(200);
         this.flame.setParticle(ParticleTypes.DRAGON_BREATH);
         this.flame.addEffect(new MobEffectInstance(MobEffects.HARM));
         this.dragon.getLevel().addFreshEntity(this.flame);
      }
