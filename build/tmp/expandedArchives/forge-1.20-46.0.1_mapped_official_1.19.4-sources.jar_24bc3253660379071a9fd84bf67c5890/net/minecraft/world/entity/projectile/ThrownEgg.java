...
public class ThrownEgg extends ThrowableItemProjectile {
   ...
   public void handleEntityEvent(byte p_37484_) {
      if (p_37484_ == 3) {
         ...
         for(int i = 0; i < 8; ++i) {
            this.getLevel().addParticle(new ItemParticleOption(ParticleTypes.ITEM, this.getItem()), this.getX(), this.getY(), this.getZ(), ((double)this.random.nextFloat() - 0.5D) * 0.08D, ((double)this.random.nextFloat() - 0.5D) * 0.08D, ((double)this.random.nextFloat() - 0.5D) * 0.08D);
         }
      }
   ...
   protected void onHit(HitResult p_37488_) {
      ...
      if (!this.getLevel().isClientSide) {
         if (this.random.nextInt(8) == 0) {
            ...
            for(int j = 0; j < i; ++j) {
               Chicken chicken = EntityType.CHICKEN.create(this.getLevel());
               if (chicken != null) {
                  chicken.setAge(-24000);
                  chicken.moveTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), 0.0F);
                  this.getLevel().addFreshEntity(chicken);
               }
            }
         }
