   ...
   public boolean canUse() {
      if (this.mob.getRandom().nextFloat() >= this.probability) {
         ...
         if (this.lookAtType == Player.class) {
            this.lookAt = this.mob.getLevel().getNearestPlayer(this.lookAtContext, this.mob, this.mob.getX(), this.mob.getEyeY(), this.mob.getZ());
         } else {
            this.lookAt = this.mob.getLevel().getNearestEntity(this.mob.getLevel().getEntitiesOfClass(this.lookAtType, this.mob.getBoundingBox().inflate((double)this.lookDistance, 3.0D, (double)this.lookDistance), (p_148124_) -> {
               return true;
