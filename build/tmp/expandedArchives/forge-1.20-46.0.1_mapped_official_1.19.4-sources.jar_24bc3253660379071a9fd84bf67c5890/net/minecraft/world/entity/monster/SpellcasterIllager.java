...
public abstract class SpellcasterIllager extends AbstractIllager {
   ...
   protected SpellcasterIllager.IllagerSpell getCurrentSpell() {
      return !this.getLevel().isClientSide ? this.currentSpell : SpellcasterIllager.IllagerSpell.byId(this.entityData.get(DATA_SPELL_CASTING_ID));
   }
   ...
   public void tick() {
      super.tick();
      if (this.getLevel().isClientSide && this.isCastingSpell()) {
         ...
         float f1 = Mth.cos(f);
         float f2 = Mth.sin(f);
         this.getLevel().addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() + (double)f1 * 0.6D, this.getY() + 1.8D, this.getZ() + (double)f2 * 0.6D, d0, d1, d2);
         this.getLevel().addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() - (double)f1 * 0.6D, this.getY() + 1.8D, this.getZ() - (double)f2 * 0.6D, d0, d1, d2);
      }
