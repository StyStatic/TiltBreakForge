...
public class ShulkerBullet extends Projectile {
   ...
   private void selectNextMoveDirection(@Nullable Direction.Axis p_37349_) {
      ...
      if (!blockpos.closerToCenterThan(this.position(), 2.0D)) {
         ...
         if (p_37349_ != Direction.Axis.X) {
            ...
            } else if (blockpos1.getX() > blockpos.getX() && this.getLevel().isEmptyBlock(blockpos1.west())) {
         ...
         if (p_37349_ != Direction.Axis.Y) {
            ...
            } else if (blockpos1.getY() > blockpos.getY() && this.getLevel().isEmptyBlock(blockpos1.below())) {
         ...
         if (p_37349_ != Direction.Axis.Z) {
            ...
            } else if (blockpos1.getZ() > blockpos.getZ() && this.getLevel().isEmptyBlock(blockpos1.north())) {
   ...
   public void tick() {
      ...
      if (!this.getLevel().isClientSide) {
         ...
         HitResult hitresult = ProjectileUtil.m_278158_(this, this::canHitEntity);
      ...
      if (this.getLevel().isClientSide) {
         this.getLevel().addParticle(ParticleTypes.END_ROD, this.getX() - vec31.x, this.getY() - vec31.y + 0.15D, this.getZ() - vec31.z, 0.0D, 0.0D, 0.0D);
         ...
         if (this.currentMoveDirection != null) {
            ...
            if (this.getLevel().loadedAndEntityCanStandOn(blockpos.relative(this.currentMoveDirection), this)) {
   ...
   protected void onHitBlock(BlockHitResult p_37343_) {
      ...
      ((ServerLevel)this.getLevel()).sendParticles(ParticleTypes.EXPLOSION, this.getX(), this.getY(), this.getZ(), 2, 0.2D, 0.2D, 0.2D, 0.0D);
      this.playSound(SoundEvents.SHULKER_BULLET_HIT, 1.0F, 1.0F);
   }

   private void destroy() {
      this.discard();
      this.getLevel().gameEvent(GameEvent.ENTITY_DAMAGE, this.position(), GameEvent.Context.of(this));
   }
   ...
   public boolean hurt(DamageSource p_37338_, float p_37339_) {
      if (!this.getLevel().isClientSide) {
         ...
         ((ServerLevel)this.getLevel()).sendParticles(ParticleTypes.CRIT, this.getX(), this.getY(), this.getZ(), 15, 0.2D, 0.2D, 0.2D, 0.0D);
         this.destroy();
      }
