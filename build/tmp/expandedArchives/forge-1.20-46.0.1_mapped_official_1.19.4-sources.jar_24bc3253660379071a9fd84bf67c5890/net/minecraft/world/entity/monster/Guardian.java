   ...
   public LivingEntity getActiveAttackTarget() {
      if (!this.hasActiveAttackTarget()) {
         ...
         if (this.clientSideCachedAttackTarget != null) {
            ...
            Entity entity = this.getLevel().getEntity(this.entityData.get(DATA_ID_ATTACK_TARGET));
         ...
         if (this.getLevel().isClientSide) {
            ...
            if (!this.isInWater()) {
               ...
               if (vec3.y > 0.0D && this.clientSideTouchedGround && !this.isSilent()) {
                  this.getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), this.getFlopSound(), this.getSoundSource(), 1.0F, 1.0F, false);
               }

               this.clientSideTouchedGround = vec3.y < 0.0D && this.getLevel().loadedAndEntityCanStandOn(this.blockPosition().below(), this);
            ...
            if (this.isMoving() && this.isInWater()) {
               ...
               for(int i = 0; i < 2; ++i) {
                  this.getLevel().addParticle(ParticleTypes.BUBBLE, this.getRandomX(0.5D) - vec31.x * 1.5D, this.getRandomY() - vec31.y * 1.5D, this.getRandomZ(0.5D) - vec31.z * 1.5D, 0.0D, 0.0D, 0.0D);
               }
            }
            ...
            if (this.hasActiveAttackTarget()) {
               ...
               if (livingentity != null) {
                  ...
                  while(d4 < d3) {
                     ...
                     this.getLevel().addParticle(ParticleTypes.BUBBLE, this.getX() + d0 * d4, this.getEyeY() + d1 * d4, this.getZ() + d2 * d4, 0.0D, 0.0D, 0.0D);
                  }
               }
            }
         }
            ...
            if (!this.guardian.hasLineOfSight(livingentity)) {
               ...
               if (this.attackTime == 0) {
                  ...
                  if (!this.guardian.isSilent()) {
                     this.guardian.getLevel().broadcastEntityEvent(this.guardian, (byte)21);
                  }
               } else if (this.attackTime >= this.guardian.getAttackDuration()) {
                  float f = 1.0F;
                  if (this.guardian.getLevel().getDifficulty() == Difficulty.HARD) {
                     f += 2.0F;
                  }
