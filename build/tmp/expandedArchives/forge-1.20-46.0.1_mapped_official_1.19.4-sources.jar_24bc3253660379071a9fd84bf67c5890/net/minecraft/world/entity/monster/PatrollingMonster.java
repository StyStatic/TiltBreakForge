...
public abstract class PatrollingMonster extends Monster {
   ...
   public static class LongDistancePatrolGoal<T extends PatrollingMonster> extends Goal {
      ...
      public boolean canUse() {
         boolean flag = this.mob.getLevel().getGameTime() < this.cooldownUntil;
      ...
      public void tick() {
         ...
         if (pathnavigation.isDone()) {
            ...
            if (this.mob.isPatrolling() && list.isEmpty()) {
               ...
               blockpos = this.mob.getLevel().getHeightmapPos(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, blockpos);
               if (!pathnavigation.moveTo((double)blockpos.getX(), (double)blockpos.getY(), (double)blockpos.getZ(), flag ? this.leaderSpeedModifier : this.speedModifier)) {
                  this.moveRandomly();
                  this.cooldownUntil = this.mob.getLevel().getGameTime() + 200L;
               } else if (flag) {
      ...
      private List<PatrollingMonster> findPatrolCompanions() {
         return this.mob.getLevel().getEntitiesOfClass(PatrollingMonster.class, this.mob.getBoundingBox().inflate(16.0D), (p_264971_) -> {
      ...
      private boolean moveRandomly() {
         ...
         BlockPos blockpos = this.mob.getLevel().getHeightmapPos(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, this.mob.blockPosition().offset(-8 + randomsource.nextInt(16), 0, -8 + randomsource.nextInt(16)));
