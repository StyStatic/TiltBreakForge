...
public class WitherBoss extends Monster implements PowerableMob, RangedAttackMob {
   ...
   public void aiStep() {
      ...
      if (!this.getLevel().isClientSide && this.getAlternativeTarget(0) > 0) {
         Entity entity = this.getLevel().getEntity(this.getAlternativeTarget(0));
         if (entity != null) {
            double d0 = vec3.y;
      ...
      for(int j = 0; j < 2; ++j) {
         ...
         Entity entity1 = null;
         if (k > 0) {
            entity1 = this.getLevel().getEntity(k);
         }
      ...
      for(int l = 0; l < 3; ++l) {
         ...
         this.getLevel().addParticle(ParticleTypes.SMOKE, d8 + this.random.nextGaussian() * (double)0.3F, d10 + this.random.nextGaussian() * (double)0.3F, d2 + this.random.nextGaussian() * (double)0.3F, 0.0D, 0.0D, 0.0D);
         if (flag && this.getLevel().random.nextInt(4) == 0) {
            this.getLevel().addParticle(ParticleTypes.ENTITY_EFFECT, d8 + this.random.nextGaussian() * (double)0.3F, d10 + this.random.nextGaussian() * (double)0.3F, d2 + this.random.nextGaussian() * (double)0.3F, (double)0.7F, (double)0.7F, 0.5D);
         }
      }

      if (this.getInvulnerableTicks() > 0) {
         for(int i1 = 0; i1 < 3; ++i1) {
            this.getLevel().addParticle(ParticleTypes.ENTITY_EFFECT, this.getX() + this.random.nextGaussian(), this.getY() + (double)(this.random.nextFloat() * 3.3F), this.getZ() + this.random.nextGaussian(), (double)0.7F, (double)0.7F, (double)0.9F);
         }
      }
   ...
   protected void customServerAiStep() {
      if (this.getInvulnerableTicks() > 0) {
         ...
         if (k1 <= 0) {
            this.getLevel().explode(this, this.getX(), this.getEyeY(), this.getZ(), 7.0F, false, Level.ExplosionInteraction.MOB);
            if (!this.isSilent()) {
               this.getLevel().globalLevelEvent(1023, this.blockPosition(), 0);
            }
         }
         ...
         for(int i = 1; i < 3; ++i) {
            if (this.tickCount >= this.nextHeadUpdate[i - 1]) {
               ...
               if (this.getLevel().getDifficulty() == Difficulty.NORMAL || this.getLevel().getDifficulty() == Difficulty.HARD) {
                  int i3 = i - 1;
               ...
               if (l1 > 0) {
                  LivingEntity livingentity = (LivingEntity)this.getLevel().getEntity(l1);
                  ...
                  List<LivingEntity> list = this.getLevel().getNearbyEntities(LivingEntity.class, TARGETING_CONDITIONS, this, this.getBoundingBox().inflate(20.0D, 8.0D, 20.0D));
                  if (!list.isEmpty()) {
         ...
         if (this.destroyBlocksTick > 0) {
            ...
            if (this.destroyBlocksTick == 0 && net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(this.getLevel(), this)) {
               ...
               for(int j = -1; j <= 1; ++j) {
                  for(int k2 = -1; k2 <= 1; ++k2) {
                     for(int k = 0; k <= 3; ++k) {
                        ...
                        if (blockstate.canEntityDestroy(this.getLevel(), blockpos, this) && net.minecraftforge.event.ForgeEventFactory.onEntityDestroyBlock(this, blockpos, blockstate)) {
                           flag = this.getLevel().destroyBlock(blockpos, true, this) || flag;
                        }
                     }
                  }
               }
               ...
               if (flag) {
                  this.getLevel().levelEvent((Player)null, 1022, this.blockPosition(), 0);
               }
            }
         }
   ...
   private void performRangedAttack(int p_31449_, double p_31450_, double p_31451_, double p_31452_, boolean p_31453_) {
      if (!this.isSilent()) {
         this.getLevel().levelEvent((Player)null, 1024, this.blockPosition(), 0);
      }
      ...
      WitherSkull witherskull = new WitherSkull(this.getLevel(), this, d3, d4, d5);
      ...
      this.getLevel().addFreshEntity(witherskull);
   }
