...
public class Warden extends Monster implements VibrationSystem {
   ...
   public Warden(EntityType<? extends Monster> p_219350_, Level p_219351_) {
      ...
      this.f_279603_ = new Warden.VibrationUser();
   ...
   public void tick() {
      ...
      if (level instanceof ServerLevel serverlevel) {
         VibrationSystem.Ticker.m_280259_(serverlevel, this.f_279590_, this.f_279603_);
      ...
      if (this.getLevel().isClientSide()) {
         if (this.tickCount % this.getHeartBeatDelay() == 0) {
            ...
            if (!this.isSilent()) {
               this.getLevel().playLocalSound(this.getX(), this.getY(), this.getZ(), SoundEvents.WARDEN_HEARTBEAT, this.getSoundSource(), 5.0F, this.getVoicePitch(), false);
            }
         }
      ...
      if ((float)p_219384_.getAccumulatedTime() < 4500.0F) {
         ...
         if (blockstate.getRenderShape() != RenderShape.INVISIBLE) {
            for(int i = 0; i < 30; ++i) {
               ...
               this.getLevel().addParticle(new BlockParticleOption(ParticleTypes.BLOCK, blockstate), d0, d1, d2, 0.0D, 0.0D, 0.0D);
            }
         }
      }
   ...
   public boolean canTargetEntity(@Nullable Entity p_219386_) {
      if (p_219386_ instanceof LivingEntity livingentity) {
         if (this.getLevel() == p_219386_.getLevel() && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(p_219386_) && !this.isAlliedTo(p_219386_) && livingentity.getType() != EntityType.ARMOR_STAND && livingentity.getType() != EntityType.WARDEN && !livingentity.isInvulnerable() && !livingentity.isDeadOrDying() && this.getLevel().getWorldBorder().isWithinBounds(livingentity.getBoundingBox())) {
            return true;
         }
      }
   ...
   public void addAdditionalSaveData(CompoundTag p_219434_) {
      ...
      VibrationSystem.Data.f_279637_.encodeStart(NbtOps.INSTANCE, this.f_279590_).resultOrPartial(LOGGER::error).ifPresent((p_219418_) -> {
   ...
   public void readAdditionalSaveData(CompoundTag p_219415_) {
      ...
      if (p_219415_.contains("listener", 10)) {
         VibrationSystem.Data.f_279637_.parse(new Dynamic<>(NbtOps.INSTANCE, p_219415_.getCompound("listener"))).resultOrPartial(LOGGER::error).ifPresent((p_281093_) -> {
   ...
   public boolean hurt(DamageSource p_219381_, float p_219382_) {
      ...
      if (!this.getLevel().isClientSide && !this.isNoAi() && !this.isDiggingOrEmerging()) {
   ...
   class VibrationUser implements VibrationSystem.User {
      ...
      private final PositionSource f_279600_ = new EntityPositionSource(Warden.this, Warden.this.getEyeHeight());
