...
public class PolarBear extends Animal implements NeutralMob {
   ...
   public void readAdditionalSaveData(CompoundTag p_29541_) {
      ...
      this.readPersistentAngerSaveData(this.getLevel(), p_29541_);
   }
   ...
   class PolarBearAttackPlayersGoal extends NearestAttackableTargetGoal<Player> {
      ...
      public boolean canUse() {
         if (PolarBear.this.isBaby()) {
            ...
            if (super.canUse()) {
               for(PolarBear polarbear : PolarBear.this.getLevel().getEntitiesOfClass(PolarBear.class, PolarBear.this.getBoundingBox().inflate(8.0D, 4.0D, 8.0D))) {
                  if (polarbear.isBaby()) {
                     return true;
                  }
               }
